...
// 			showTarget();
			...
			$('#publishType').change(function(){
				location.href = "${path}/publishRes/offlinePubMain.jsp?publishType="+$('#publishType').val();
			});
			queryForm();
...
// 				alert($('#lifeCycle').val());
		...
		function onChecks(event, treeId, treeNode) {
		    ...
		    document.getElementById("queryForm").action = '${path}/target/gotoTargetList.action?publishType='+$('#publishType').val()+'&targetNames='+targetNames+"&targetField="+$('#targetField').val()+'&isTarget=1&offline=1';
...
// 			}
		function queryForm(){
			...
			var url = 'offlinePublishResList.action?publishType='+$('#publishType').val();
 			url +=<app:MainPageParameterTag />+"&targetField="+$('#targetField').val();
		...
		function queryClass(){
			...
			$.ajax({
				...
				success : function(data) {
					...
					$('#classPage').attr('src',"${path}/publishRes/class.jsp?fieldName="+firstName+"&typeId="+firstID+"&offline=1");
				}
			});
		}
		...
		function showClass() {
			...
			$('#classPage').attr('src',"${path}/publishRes/class.jsp?fieldName="+fieldName+"&typeId="+typeId+"&offline=1");
		}
...
// 			$.ajax({
// 				url :'${path}/target/getTargetsForName.action?targetName='+targetName+"&publishType="+$('#publishType').val(),
// 				type : 'post',
// 				async : true,
// 				success : function(data) {
// 					var publishTypeList = $("#tab_1_1_4");
			...
			<div class="by-tool-box">
				<div class="by-form-row clearfix">
				    ...
				    <div class="by-form-val">
				        <app:selectResType name="publishType" id="publishType" selectedVal="${param.publishType}"  headName=""  headValue=""  />
				    </div>
				</div>
				<div class="by-tab">
