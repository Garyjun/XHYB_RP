...
<!--                                  style="width:210%" -->
           ...
           <c:if test="${detailFlag!='detail'}">
	        ...
	        <c:if test="${detailFlag=='detail'&& flagSta==null}">
	             <sec:authorize url='WF_pubresCheck'>
				    <%@ include file="/bres/PubWFOperate.jsp" %>
				    <div class="form-actions" align="center">
				      <c:if test="${searchFlag=='close'}">
				     <input class="btn btn-primary" type="button" value="关闭" onclick="javascript:$.closeFromInner();"/>
				      </c:if>
				   <c:if test="${empty wfTaskId && copyrightWaring==''}">
				   <button type="button" class="btn btn-lg blue" onclick="goBackTask();">返回</button>
				   </c:if>
				     </div>
				 </sec:authorize>
	        </c:if>
	        <c:if test="${copyrightWaring!=''&&copyrightWaring!=null}">
	        <div class="form-actions" align="center">
				   <button type="button" class="btn btn-lg blue" onclick="goBackTaskWaring();">返回</button>
		    </div>
			</c:if>
	        <c:if test="${detailFlag=='detail'&& flagSta=='2'}">
	         <sec:authorize url='WF_pubresCheck'>
				    <%@ include file="/bres/PubWFOperate.jsp" %>
				    <div class="form-actions" align="center">
				    <c:if test="${searchFlag=='close'}">
	        ...
	        <c:if test="${detailFlag=='detail'&& flagSta=='3'}">
	         <sec:authorize url='WF_pubresCheck'>
				    <%@ include file="/bres/PubWFOperate.jsp" %>
				    ...
				    <button type="button" class="btn btn-lg blue" onclick="goBackTask();">返回</button>
				      ...
				      <c:if test="${empty wfTaskId&&param.buttonShow==undefined}">
				     <input class="btn btn-primary" type="button" value="关闭" onclick="javascript:$.closeFromInner();"/>
				       </c:if>
				     </div>
				 </sec:authorize>
...
<!--  <button type="button"  class="btn btn-lg blue" onclick="save('add')" >提交</button> -->
 </form>
 <script type="text/javascript">
 	function goBackTask(){
