...
<script type="text/javascript">
	...
	function save(page,page1,pageSize){
        ...
        var ftpHttpDate = {
        		...
        		metadataMap:params,						//页面查询条件
    			batch:"1"                     //标识为查询统计处的导出，为了区分与资源管理不同处的导出，有逻辑不同
	    };
        
		var ftpHttp = encodeURI(JSON.stringify(ftpHttpDate));
		$.ajax({
			url :'${path}/acrossStoreSearch/downloadPageResource.action?ftpFlag=2',
			type : 'post',
			async : true,            
...
//		alert(flagDown[0]);
	...
	function gotoDownSucess(){
		...
		window.location.href = "${path}/acrossStoreSearch/downloadFile.action?zipName="+zipName;
//		$.closeFromInner();
	}
