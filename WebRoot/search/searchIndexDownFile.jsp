...
//元数据提交
 $(function() {
     ...
     $('input[name="compress"]').change(function(){
    	 ...
    	 if(compressType == '1'){  //表示压缩
    		 $('#encryptPwd').attr("disabled",false);
    	 }else if(compressType == '2'){  //表示不压缩
    		 $('#encryptPwd').val("");
 	...
 	function save(){
		...
		var isComplete = $('input:radio[name="compress"]:checked').val();
 		...
 		$.ajax({
			url :'${path}/search/downloadPublishRes.action',
			type : 'post',
			async : true,
...
// 		alert(flagDown[0]);
	...
	function gotoDownSucess(){
		...
		window.location.href = "${path}/search/downloadFile.action?zipName="+zipName;
// 		$.closeFromInner();
	}
