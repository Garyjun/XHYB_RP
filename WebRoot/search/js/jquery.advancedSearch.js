...
for(var count=1;count<=conditonNum.length;count++){
	...
	if(fieldType==2||fieldType==3||fieldType==4){
		...
		//alert("多选值为："+ val); 
	}else if(fieldType==1||fieldType==5||fieldType==9){
	...
	$(fieldVals).each(function(index){
		...
		if(operatorValue=="equ"){
			if(size==1){
				...
				if(i==size){
					params += val + ")" + " " + contactWord + " ";
				}else if(i==1){
					params += fieldEnName + ":(" + val + ",";
				}else{
					params += val + ",";
				}
			}
...
document.getElementById("myForm").reset();//输入框采用form的reset方法
$('.selectpicker').selectpicker('deselectAll');//清空选项值，不能用传统jquery
	var params = getAllQueryConditions();
	queryForm(params);
}
