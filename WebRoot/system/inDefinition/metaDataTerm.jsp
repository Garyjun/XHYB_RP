	...
	<script type="text/javascript">
	   ...
	   function queryMedata(){
			...
			function save(){
				   ...
				   $.post('${path}/system/inDefinition/addMetaAction.action',
						  {nameMust:must,nameMay:may,nameMustCN:mustcn,nameMayCN:maycn,pid:metaDataTermId},
						  function(data){
			...
			$(function(){
				$('#checkall').click(function(){
					if($('input[name="must"]').attr("checked")!= "checked"){
						$('input[name="must"]').attr("checked", "checked");
                    }else {
                    	$('input[name="must"]').removeAttr("checked");
                    	}
				});
			...
			$(function(){
				$('#Mcheckall').click(function(){
					if($('input[name="may"]').attr("checked")!= "checked"){
						$('input[name="may"]').attr("checked", "checked");
                    }else {
                    	$('input[name="may"]').removeAttr("checked");
                    	}
				});
	...
	<div id="fakeFrame" class="container-fluid by-frame-wrap" style="height: 100%;">
		...
		<div class="portlet portlet-border">
	        <div class="portlet-title">
	            <div class="caption"><span>必选数据元素</span>
	             <div style='float:right;'> 
	              <input id='checkall' type='checkbox'>全选</input>
	             </div>
	            </div>
	        </div>
		...
		<div class="portlet portlet-border">
	        <div class="portlet-title">
	            <div class="caption"><span>可选数据元素</span>
	            <div style='float:right;'> 
	              <input id='Mcheckall' type='checkbox'>全选</input>
	             </div>
	            </div>
	        </div>
            <div class="portlet-body">
