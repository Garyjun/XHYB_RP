...
<head>
	...
	function importExcel(){
		$.openWindow("${path}/system/dataManagement/classification/importClassicExcel.jsp?moudleName=tzzy",'导入分类体系',450,200);
	}
    ...
    function addHoverDom(treeId, treeNode) {
        ...
        if ($("#setAliasBtn_"+treeNode.tId).length==0&&treeNode.level==0){
        	var setAliasStr = "<sec:authorize url='/system/dataManagement/classification/tbzy/deleteNode.action'><span class='button' id='setAliasBtn_" + treeNode.tId
        ...
        if ($("#removeBtn_"+treeNode.tId).length==0){
        	var removeStr = "<sec:authorize url='/system/dataManagement/classification/tzzy/deleteNode.action'><span class='button remove' id='removeBtn_" + treeNode.tId
        ...
        if ($("#addBtn_"+treeNode.tId).length==0){
			var addStr = "<sec:authorize url='/system/book/tzzy/getSelectValues.action'><span class='button add' id='addBtn_"
    ...
    $(document).ready(function(){
        var t = $("#tree");
		$.get("listClassification.action?moudleName=TZ",
			function(data) {
				var books = eval("("+data+")");
	...
	function saveNode() {
		...
		}else if($("#operation").val()=='setAlias'){
			//根节点设置别名
			node = t.getNodes()[0];
			node.alias = $("#labelName").val();
		}
		//提交修改
...
<body>
	...
	<input id="moudleName" type="hidden" value="TZ"/>
</body>
</html>