...
// 	var objectIds = '${param.objectIds}';
	...
	$(function() {
		...
		dt.setPK('id');
		dt.setSortName('id');
		dt.setURL('${path}/company/query.action');
		dt.setSortOrder('desc');
		dt.setSelectBox('id');
		dt.setColums([
		{field:'name',title:'公司名称',width:fillsize(0.27),align:'left'},
		{field:'address',title:'公司地址',width:fillsize(0.27),align:'left'},
// 		{field:'city',title:'所属城市',width:fillsize(0.27),align:'left'},
// 		{field:'companyType',title:'单位类型',width:fillsize(0.27),align:'left'},
		{field:'contactFirst',title:'联系电话',width:fillsize(0.27),align:'center'},
// 		{field:'opt1',title:'操作',width:fillsize(0.27),align:'center',formatter:$operate,align:'center'}
	...
	function queryForm(params) {
		...
		dt.dataGridObj.datagrid({
					url : "${path}/company/query.action?name="+name+"&address="+address
				}); 
	}
		...
		function addRelations(ids){
			...
			if(valueLength!=""){
				if(arr.length>valueLength){
					if(ckeck == '2'){
						layer.alert("所选公司数量必须小于等于"+valueLength+"个");
					}else{
						$.alert("所选公司数量必须小于等于"+valueLength+"个");
					}
					return;
				}
			}
// 			var resId =$('#objectIds').val();
			$.post('${path}/company/searchName.action',{ids:ids},function(data){
				if(data!=""){
					if(check == '1'){
...
// 	    		 $.openWindow("${path}/staff/toEdit.action?id="+id,'修改人员信息',800,600);
	    		 var d = art.dialog.open("${path}/company/toEdit.action?id="+id,
	    			     {
	    				   lock:true,
	    			       title: "修改单位信息",
	    			       width: "800px",
...
// 	    		 $.openWindow("${path}/staff/toEdit.action?id="+id+"&fromPeopleUnit=1",'添加人员信息',800,600);
	    		 var d = art.dialog.open("${path}/company/toEdit.action?id="+id+"&fromPeopleUnit=1",
	    			     {
	    				   lock:true,
	    			       title: "添加单位信息",
	    			       width: "800px",
