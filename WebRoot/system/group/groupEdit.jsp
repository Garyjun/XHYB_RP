		...
		src="${path}/appframe/util/appDataGrid.js"></script>    
          <script type="text/javascript">
 			...
 			$(document).ready(function(){
 		   		...
 		   		//定义一datagrid
 		   		dt.setConditions(['userName']);
 		   		dt.setPK('id');
 		   		dt.setSortName('id');
 		   		dt.setURL('${path}/system/group/listUser.action?groupId='+$("#groupId").val()+"&userIds="+$("#addUserIds").val());
 		   		dt.setSortOrder('desc');
 		   		dt.setSelectBox('id');
 		   		dt.setColums([{field:'loginName',title:'登录名',width:fillsize(0.2),sortable:true},
			...
			function delUser(){
				...
				if(groupId&&groupId.length>0){
					$.get("${path}/system/group/delUser.action?ids="
							+ids+"&groupId="+groupId,function(data){
		 				if(data=="0"){
							query();
		 				}else{
		 					$.alert("删除用户组出错！");
					...
					var selectIds = getChecked('data_div','id');
					if(addUserIds!='' && addUserIds.length>0){
						addUserIds +=",";
					}
					for(var i=0;i<selectIds.length;i++){
						addUserIds = addUserIds.replace(selectIds[i]+",","");
				    }
					if(addUserIds!="" && addUserIds.length>0 && addUserIds.endWith(",")){
			...
			function returnList(){
		     ...
		     function query(){
		 		...
		 		dt.dataGridObj.datagrid({
					url : '${path}/system/group/listUser.action?groupId='+groupId+'&userIds='+userIds
				});
			}			
