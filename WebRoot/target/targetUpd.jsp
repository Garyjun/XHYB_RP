...
// 		}
		...
		function targetType(flagSelect){
			$.ajax({
				type : "post",
				async:false,
				...
				var title = document.getElementById('targetType');
				for(var j=0;j<title.options.length;j++){
					if(title.options[j].value == flagSelect){
	...
	<div id="fakeFrame" class="container-fluid by-frame-wrap" style="height: 100%;">
		...
		<form:form action="" id="form" name="form"  modelAttribute="frmWord" method="post" class="form-horizontal" role="form">
			<div class="form-group">
				   ...
				   <div class="col-xs-5">
				        <app:selectResType name="module123" id="module123" selectedVal="${module}"  headName=""  headValue=""  readonly="readonly" />
				    </div>
			</div>
			<div class="form-group">
				<label class="col-xs-5 control-label text-right"><span class="required">*</span>标签名称：</label>
				<div class="col-xs-5">
					<form:input path="targetName"  class="form-control validate[required, maxSize[50],ajax[validateTargetNameEdit]] text-input" readonly="true"/>
				</div>
			</div>
