...
function initTable(searchParam){
	...
	if(resType!=="" && resType==='0'){//期刊
		hiddenOtherTable(resType);
		_table = 'journal_list';
	}else if(resType!=="" && resType==='1'){//文章
		hiddenOtherTable(resType);
		_table = 'article_list';
	}else if(resType!=="" && resType==='2'){//大事记
		hiddenOtherTable(resType);
		_table = 'event_list';
	}else if(resType!=="" && resType==='4'){//网页爬虫
		hiddenOtherTable(resType);
		_table = 'crawl_list';
	}
	
	$.fn.dataTable.ext.errMode = "throw";//出现错误时，抛掉 默认为alert弹出
	var table = queryDataTable(_table,searchParam);//封装的查询组件
...
function onclickColumn(table,resType){
	...
	$('.display tbody').on( 'click', 'tr', function (event) {
		...
		//打开tab页面
		if(resType==="4"){
			openTabs(table.row( this ).data().sn,resType,table.row( this ).data().title,table.row( this ).data().url);
		}else{
 ...
 * @returns
 */
function queryDataTable(_table,searchParam) {
	var columnsJson = searchParam.columns;
	var resType = searchParam.resType;
	...
	for(var o in columnsJson){
		//console.info(o);
		//console.info(columnsJson[o]);
		if("content"===columnsJson[o].key || resType==='4'){
			_columns.push({"title":columnsJson[o].value,"data":columnsJson[o].key});
		}else{
			_columns.push({"title":columnsJson[o].value,"data":"metadataMap."+columnsJson[o].key});
		}
	}
