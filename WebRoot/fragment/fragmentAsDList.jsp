...
<script type="text/javascript">
	$(function(){
		...
		var _pk = 'id';
		var _conditions = ['resourceName','wordName','fragType','treeName'];
		var _sortName = 'id';
		var _sortOrder = 'desc';
		var _check=true;
		var _colums = [
				...
				{ title:'所属文章', field:'metadataMap.title' ,width:fillsize(0.07), align:'center',formatter:$wenname },
				{ title:'碎片标题', field:'metadataMap.spParagraph' ,width:fillsize(0.1), align:'center' },
...
// 		opt += "<a class=\"btn hover-red\" href=\"javascript:edit('"+rec.objectId+"')\" ><i class=\"fa fa-edit\"></i> 修改</a>";
	...
	$wenname = function(value,rec){
		var wenname='';
		var wennames = rec.metadataMap.title;
		if(wennames!=null && wennames != ""){
			if(wennames.length>=10){
				wennames = wennames.substring(0,10)+"...";
			}
		}
		wenname = "<span data-toggle='tooltip' data-placement='top' title='"+rec.metadataMap.wenname+"' >"+wennames+"</span>";
		return 	wenname;
	}
	$content = function(value,rec){
		var contents=rec.metadataMap.description;
		var tempValue = '';
	...
	function detail(objectId){
		window.location.href = "${path}/fragment/toFragmentDetail.action?objectId="+objectId+"&flagSta=4&publishType=76"+"&buttonShow=1";
	}
	function queryList(){
		$grid.query();
	}
...
<body>
	...
	<input type="hidden" id="treeName" name="treeName" value="${param.treeName }"/>
</body>
</html>