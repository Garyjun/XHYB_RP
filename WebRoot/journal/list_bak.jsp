    ...
    <head>
		...
		$(function(){
			...
			var level = $("#level").val();
 	   		//定义一datagrid
 	   		var _divId = 'data_div';
 	   		var _url = '${path}/journal/list.action';
 	   		var _pk = 'id';
 	   		var _conditions = ['code','level'];
 	   		var _sortName = 'id';
 	   		var _sortOrder = 'desc';
 	   		var _check=true;
 			var _colums = [ 
 						    ...
 						    { title:'当前期数', field:'metadataMap.magazineYear' ,width:100, align:'center' },
 						    { title:'总期数', field:'metadataMap.magazineNum' ,width:100, align:'center' },
 						    { title:'发布时间', field:'metadataMap.pressdate' ,width:100, align:'center' },
		...
		/***操作列***/
 		$operate = function(value,rec){
 			var opt = "";
 		    opt += '<sec:authorize url="/role/view/*"><a class=\"btn hover-red\"  href="javascript:view('+rec.id+')"><i class=\"fa fa-sign-out\"></i>详细</a>&nbsp;</sec:authorize> ';		
		    opt += '<sec:authorize url="/role/update*"><a  class=\"btn hover-red\" href="javascript:edit('+rec.id+')"><i class=\"fa fa-edit\"></i>修改</a>&nbsp;</sec:authorize> ';	
		    opt += '<sec:authorize url="/role/delete/*"><a class=\"btn hover-red\" href="javascript:del('+rec.id+');"><i class=\"fa fa-trash-o\"></i>删除</a>&nbsp;</sec:authorize>';	
 			return opt;
 		};
					...
					<div style="float: right;">
						<form action="#" class="form-inline no-btm-form" role="form" id="queryForm">
						   <div class="form-group">
							      <input type="text" class="form-control" id="roleName" name="roleName"  placeholder="当前期数" />
							</div>							
