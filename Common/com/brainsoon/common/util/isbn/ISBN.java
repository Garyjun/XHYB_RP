...
public final class ISBN {
     ...
     * @param String isbn：需要进行校验的ISBN字符串
      ...
      */
    public static boolean checkISBN(String isbn) throws InvalidStandardIDException{
        ...
        case 10:
            ...
            // 取出前9位数字进行加权和计算            
              for (int i = 0; i < 9; i++) {
                // 若前9位数据中有非数字字符，则抛出异常
                  if (cs[i] < '0' || cs[i] > '9') {
                    throw new InvalidStandardIDException("ISBN " + isbn +
                            " 第 " + (i + 1) + " 位中出现非法字符 " + cs[i]);
                }
        ...
        case 13:
            ...
            // ISBN为13位数据时，前3位目前只能是“978”（已实行）或“979”（暂未实行）
              if (!isbn.startsWith("978") && !isbn.startsWith("979")) {
                throw new InvalidStandardIDException("ISBN-13 格式不符合标准");
            }
            // 取出前12位数字进行加权和计算
              int countEven = 0;
            int countOdd = 0;
        ...
        default:
            // ISBN为非10位或13位时抛出异常
             throw new InvalidStandardIDException("ISBN 格式不符合标准");
        }
    }
}
