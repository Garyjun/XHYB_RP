 ...
 *
 */
public class AssetMetadataUtil {
	...
	public static StringBuffer createInput(StringBuffer sb,String flag,
		...
		if (en != null && en.getObjectId() != null) {
			Map<String, String> metaDateMap = en.getMetadataMap();
...
//			}
			 if(childMap != null){
		        	for(Iterator it = childMap.entrySet().iterator();it.hasNext();){ 
		        		boolean isSelected = false;
		        		java.util.Map.Entry<Object, String> entry = (java.util.Map.Entry<Object, String>)it.next();
			            if(StringUtils.isNotBlank(value)){
			            	for(String oneValue:mulValue){
...
//							}
	 ...
	 * @return
	 */
	public static StringBuffer createTextarea(StringBuffer sb,String flag,MetadataDefinition metadataDefinition,Object object,boolean isEdit,String basePath,String flagImage){
		Asset en =null;
		Map fileMap = null;
		String[] mulValue = null;
		@SuppressWarnings("unused")
		...
		if(isEdit){
			...
			if(StringUtils.isNotBlank(flagImage)){
				sb.append("<img src='"+basePath+"fileDir/fileRoot/期刊/"+value+"' class='Img' width='100' height='120' alt='封面不存在' onclick='openImg(\""+basePath+"/fileDir/fileRoot/期刊/"+value+"\");'/>");
			}else{
